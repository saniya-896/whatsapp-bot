...
# ---------------- VOICE SUPPORT ----------------
    ...
    if num_media>0:
        ...
        if "audio" not in content_type:
            msg.body("Send a voice message")
            return str(resp)

        media_url=request.values.get("MediaUrl0")

        audio=requests.get(media_url,auth=HTTPBasicAuth(ACCOUNT_SID,AUTH_TOKEN))
        ...
        try:
            ...
            try:
                text_msg=r.recognize_google(audio_data,language="ml-IN").lower()
            except:
                text_msg=r.recognize_google(audio_data,language="en-IN").lower()

            print("VOICE:",text_msg)
...
# ---------------- ADMIN ----------------
    ...
    if sender in ADMIN_NUMBERS:
        ...
        if text_msg=="admin":
            ...
            if not os.path.exists("applications.csv"):
                msg.body("No applications")
                return str(resp)

            with open("applications.csv") as f:
                rows=list(csv.reader(f))

            txt="Recent Applications\n\n"
